...
class TestPiiMasking:
    ...
    async def test_email_is_redacted(
        ...
        result: SanitizationResult = await sanitize(
            prompt, tenant_config, settings
        )
    ...
    async def test_ssn_is_redacted(
        ...
        assert (
            "US_SSN" in result.pii_detected
            or "CREDIT_CARD" in result.pii_detected
        )
        assert "123-45-6789" not in result.sanitized_text

    @pytest.mark.asyncio
    async def test_redaction_map_populated(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_no_pii_prompt_passes_cleanly(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_pii_email_not_sent_to_provider(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
    ) -> None:
        """PII must be redacted from sanitized_text before forwarding."""
        prompt = "My email is alice@example.com, please help me."
        result = await sanitize(prompt, tenant_config, settings)
        assert "alice@example.com" not in result.sanitized_text
...
class TestInjectionDetection:
    ...
    @pytest.mark.asyncio
    async def test_injection_score_is_float_in_range(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_benign_prompt_not_blocked(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_obvious_injection_scored(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
        injection_prompt: str,
    ) -> None:
        """Injection score must be computed (>= 0) for known phrases."""
...
class TestSanitizeContract:
    ...
    @pytest.mark.asyncio
    async def test_result_is_sanitization_result_type(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_empty_prompt_does_not_raise(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_very_long_prompt_handled(
        self,
        tenant_config: TenantConfig,
        settings: SentinelSettings,
    ) -> None:
