...
@router.get('')
async def list_policies(request: Request, framework: Optional[str]=None, status: Optional[str]=None, page: int=1, db=None):
...
async def list_frameworks(request: Request, db=None):
    tenant_id = get_tenant(request)
...
async def update_notif_settings(request: Request, body: NotifSettingsBody, db=None):
    tenant_id = get_tenant(request)
    await db.execute('INSERT INTO policy_notification_settings(tenant_id) VALUES() ON CONFLICT DO NOTHING', tenant_id)
...
async def public_view(token: str, db=None):
    try:
...
async def get_policy(policy_id: str, request: Request, db=None):
    tenant_id = get_tenant(request)
...
async def get_versions(policy_id: str, request: Request, db=None):
    tenant_id = get_tenant(request)
...
async def get_activity(policy_id: str, request: Request, page: int=1, db=None):
    tenant_id = get_tenant(request)
...
async def get_signatures(policy_id: str, request: Request, db=None):
    tenant_id = get_tenant(request)
...
async def download_pdf(policy_id: str, request: Request, db=None):
    tenant_id = get_tenant(request)
    user_id = get_user(request)
...
async def clone_template(template_id: str, request: Request, db=None):
    tenant_id = get_tenant(request)
    user_id = get_user(request)
    return await engine.clone_system_template(template_id, tenant_id, user_id, db)

@router.post('')
async def create_policy(request: Request, body: CreatePolicyBody, db=None):
    tenant_id = get_tenant(request)
    user_id = get_user(request)
    return await engine.create_policy(tenant_id, body.framework, body.title, body.body, body.review_frequency, user_id, db)

@router.patch('/{policy_id}')
async def edit_policy(policy_id: str, request: Request, body: EditPolicyBody, db=None):
    tenant_id = get_tenant(request)
    user_id = get_user(request)
...
async def submit_review(policy_id: str, request: Request, body: SubmitBody, db=None):
    tenant_id = get_tenant(request)
    user_id = get_user(request)
    return await engine.submit_for_review(policy_id, body.reviewer_id, user_id, tenant_id, db)

@router.post('/{policy_id}/approve')
async def approve_policy(policy_id: str, request: Request, body: ApproveBody, db=None):
    tenant_id = get_tenant(request)
...
async def reject_policy(policy_id: str, request: Request, body: RejectBody, db=None):
    tenant_id = get_tenant(request)
    actor_id = get_user(request)
...
async def archive_policy(policy_id: str, request: Request, body: ArchiveBody, db=None):
    tenant_id = get_tenant(request)
    actor_id = get_user(request)
    return await engine.archive_policy(policy_id, body.reason, actor_id, tenant_id, db)

@router.post('/{policy_id}/sign')
async def sign_policy(policy_id: str, request: Request, body: SignBody, db=None):
    tenant_id = get_tenant(request)
    user_id = get_user(request)
...
async def share_policy(policy_id: str, request: Request, body: ShareBody, db=None):
    tenant_id = get_tenant(request)
    actor_id = get_user(request)
