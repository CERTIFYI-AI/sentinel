...
# Tenant-scoped CRUD for TrustTrace.
#
...
from sentinel.api.db import get_db, TrustTrace, new_id
...
_WRITABLE = {c.name for c in TrustTrace.__table__.columns} - {"id", "tenant_id", "created_at", "updated_at"}
...
async def list_items(skip: int = 0, limit: int = 100,
    ...
    skip = max(0, skip)
    r = await db.execute(
        select(TrustTrace).where(TrustTrace.tenant_id == tenant)
        .order_by(desc(TrustTrace.created_at)).offset(skip).limit(limit)
    )
...
async def create_item(data: dict,
                      tenant: str = Depends(get_current_tenant_id),
                      db: AsyncSession = Depends(get_db)):
    item = TrustTrace(id=new_id(), tenant_id=tenant, **_clean(data))
    db.add(item)
    await db.flush()
    await emit("trust.score.computed", "trust_engine", payload={"id": item.id})
    return _dump(item)
...
async def get_item(item_id: str,
                   tenant: str = Depends(get_current_tenant_id),
                   db: AsyncSession = Depends(get_db)):
    r = await db.execute(select(TrustTrace).where(TrustTrace.id == item_id, TrustTrace.tenant_id == tenant))
    item = r.scalar_one_or_none()
    if not item:
        raise HTTPException(404, "TrustTrace not found")
    return _dump(item)
...
async def update_item(item_id: str, data: dict,
    ...
    for k, v in _clean(data).items():
        setattr(item, k, v)
    await db.flush()
    await emit("trust.config.updated", "trust_engine", payload={"id": item_id})
    return _dump(item)
