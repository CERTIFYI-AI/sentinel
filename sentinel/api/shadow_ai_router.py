...
# Tenant-scoped CRUD for ShadowAIFinding.
#
...
from sentinel.api.db import get_db, ShadowAIFinding, new_id
...
_WRITABLE = {c.name for c in ShadowAIFinding.__table__.columns} - {"id", "tenant_id", "created_at", "updated_at"}
...
async def list_items(skip: int = 0, limit: int = 100,
    ...
    skip = max(0, skip)
    r = await db.execute(
        select(ShadowAIFinding).where(ShadowAIFinding.tenant_id == tenant)
        .order_by(desc(ShadowAIFinding.created_at)).offset(skip).limit(limit)
    )
...
async def create_item(data: dict,
                      tenant: str = Depends(get_current_tenant_id),
                      db: AsyncSession = Depends(get_db)):
    item = ShadowAIFinding(id=new_id(), tenant_id=tenant, **_clean(data))
    db.add(item)
    await db.flush()
    await emit("shadow_ai.detected", "shadow_ai", payload={"id": item.id})
    return _dump(item)
...
async def get_item(item_id: str,
                   tenant: str = Depends(get_current_tenant_id),
                   db: AsyncSession = Depends(get_db)):
    r = await db.execute(select(ShadowAIFinding).where(ShadowAIFinding.id == item_id, ShadowAIFinding.tenant_id == tenant))
    item = r.scalar_one_or_none()
    if not item:
        raise HTTPException(404, "ShadowAIFinding not found")
    return _dump(item)
