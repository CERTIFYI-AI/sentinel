...
# Tenant-scoped CRUD for VendorQuestionnaire.
#
...
from sentinel.api.db import get_db, VendorQuestionnaire, new_id
...
_WRITABLE = {c.name for c in VendorQuestionnaire.__table__.columns} - {"id", "tenant_id", "created_at", "updated_at"}
...
async def list_items(skip: int = 0, limit: int = 100,
    ...
    skip = max(0, skip)
    r = await db.execute(
        select(VendorQuestionnaire).where(VendorQuestionnaire.tenant_id == tenant)
        .order_by(desc(VendorQuestionnaire.created_at)).offset(skip).limit(limit)
    )
...
async def create_item(data: dict,
                      tenant: str = Depends(get_current_tenant_id),
                      db: AsyncSession = Depends(get_db)):
    item = VendorQuestionnaire(id=new_id(), tenant_id=tenant, **_clean(data))
    db.add(item)
    await db.flush()
...
async def get_item(item_id: str,
                   tenant: str = Depends(get_current_tenant_id),
                   db: AsyncSession = Depends(get_db)):
    r = await db.execute(select(VendorQuestionnaire).where(VendorQuestionnaire.id == item_id, VendorQuestionnaire.tenant_id == tenant))
    item = r.scalar_one_or_none()
    if not item:
        raise HTTPException(404, "VendorQuestionnaire not found")
    return _dump(item)
