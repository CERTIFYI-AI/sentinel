...
from sentinel.api.db import get_db, ObservabilityMetric, new_id, now
...
async def list_items(skip: int=0, limit: int=100, db: AsyncSession=Depends(get_db)):
    r = await db.execute(select(ObservabilityMetric).order_by(desc(ObservabilityMetric.created_at)).offset(skip).limit(limit))
    items = r.scalars().all()
...
async def create_item(data: dict, db: AsyncSession=Depends(get_db)):
    item = ObservabilityMetric(id=new_id(), **{k:v for k,v in data.items() if k != "id"})
    db.add(item)
    await db.flush()
...
async def get_item(item_id: str, db: AsyncSession=Depends(get_db)):
    r = await db.execute(select(ObservabilityMetric).where(ObservabilityMetric.id==item_id))
    item = r.scalar_one_or_none()
    if not item: raise HTTPException(404, "ObservabilityMetric not found")
    return {k:v for k,v in item.__dict__.items() if not k.startswith("_")}
