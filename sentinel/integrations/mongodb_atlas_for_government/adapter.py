...
"""MongoDB Atlas for Government integration adapter.

Reads cluster and organization security posture from the Atlas for
Government Administration API: organization API keys with owner-level
roles, project-level IP access-list exposure, and encryption-at-rest key
management configuration.

Auth: HTTP Digest, using an Atlas for Government API public key as the
digest username and the paired private key as the digest password. This
adapter is intentionally independent of ``mongodb_atlas`` — same API
shape, different (FedRAMP-authorized) host — so a commercial-tenant
credential can never be pointed at government data by mistake, and vice
versa.
"""
...
_BASE = "https://cloud.mongodbgov.com/api/atlas/v2"
...
@dataclass
class MongoDbAtlasForGovernmentCredentials:
    """Matches dashboard/src/integrations/mongodb_atlas_for_government/config.ts credentialFields."""

    public_key: str
    private_credential: str


class MongoDbAtlasForGovernmentAdapter:
    """Reads organization and project security posture from MongoDB Atlas for Government."""

    def __init__(
        self, credentials: MongoDbAtlasForGovernmentCredentials, client: httpx.AsyncClient | None = None
    ) -> None:
    ...
    async def validate(self) -> bool:
        ...
        try:
            ...
            if resp.status_code in (401, 403):
                raise ValueError(
                    "Atlas for Government rejected the API key pair. Verify "
                    "the public key and private key are active and have not "
                    "been revoked."
                )
            ...
            raise ValueError(f"Could not reach MongoDB Atlas for Government: {exc}") from exc
        finally:
    ...
    async def fetch_all(self) -> list[IntegrationFinding]:
        ...
        for result in results:
            if isinstance(result, BaseException):
                logger.warning("mongodb_atlas_for_government check failed: %s", result)
                continue
    ...
    async def _check_org_owner_api_keys(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        ...
        if not orgs:
            return [self._unavailable(
                "mongodb_atlas_gov.org.owner_api_keys",
        ...
        return [IntegrationFinding(
            check_id="mongodb_atlas_gov.org.owner_api_keys",
            ...
            remediation="Atlas for Government → Organization Access Manager → API Keys → "
                        "scope keys to the narrowest role that does the job (e.g. "
                        "ORG_READ_ONLY, PROJECT_OWNER on a specific project) instead of ORG_OWNER.",
            status=status,
    ...
    async def _check_project_network_access(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        ...
        if not projects:
            return [self._unavailable(
                "mongodb_atlas_gov.project.network_access_open",
        ...
        return [IntegrationFinding(
            check_id="mongodb_atlas_gov.project.network_access_open",
            ...
            remediation="Atlas for Government → Project → Network Access → remove the "
                        "0.0.0.0/0 entry and add specific CIDR ranges, or use "
                        "VPC/Private Endpoint peering.",
            status=status,
    ...
    async def _check_encryption_at_rest(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        ...
        if not projects:
            return [self._unavailable(
                "mongodb_atlas_gov.project.encryption_at_rest",
        ...
        return [IntegrationFinding(
            check_id="mongodb_atlas_gov.project.encryption_at_rest",
            ...
            remediation="Atlas for Government → Project → Security → Advanced → Encryption "
                        "at Rest using Customer Key Management → configure a KMS key for "
                        "the project's cloud provider.",
            status=status,
            severity="INFO" if status == "PASSED" else "MEDIUM",
    ...
    def _unavailable(check_id: str, title: str, category: str, remediation: str) -> IntegrationFinding:
        return IntegrationFinding(
            ...
            description="Sentinel could not read this from MongoDB Atlas for Government "
