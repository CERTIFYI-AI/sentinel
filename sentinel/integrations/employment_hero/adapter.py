...
"""Employment Hero integration adapter.

Reads joiner-mover-leaver (JML) evidence from the Employment Hero API:
...
_AUTH_URL = "https://oauth.employmenthero.com/oauth2/token"
_BASE = "https://api.employmenthero.com/api/v1"


@dataclass
class EmploymentHeroCredentials:
    """Matches dashboard/src/integrations/employment_hero/config.ts credentialFields."""

    client_id: str
    client_credential: str


class EmploymentHeroAdapter:
    """Fetches JML roster evidence from Employment Hero."""

    def __init__(self, credentials: EmploymentHeroCredentials, client: httpx.AsyncClient | None = None) -> None:
    ...
    async def _authenticate(self, client: httpx.AsyncClient) -> str:
        ...
        if resp.status_code in (401, 403):
            raise ValueError(
                "Employment Hero rejected the OAuth2 credentials "
    ...
    async def validate(self) -> bool:
        ...
        try:
            ...
            if resp.status_code in (401, 403):
                raise ValueError(
                    "Employment Hero rejected the request. Verify the "
                    "OAuth2 client has read permissions on employees."
                )
            resp.raise_for_status()
            return True
        except ValueError:
            ...
            raise ValueError(f"Could not reach Employment Hero: {exc}") from exc
        finally:
    ...
    async def fetch_all(self) -> list[IntegrationFinding]:
        ...
        for result in results:
            if isinstance(result, BaseException):
                logger.warning("employment hero check failed: %s", result)
                continue
    ...
    async def _check_terminated_still_active(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        ...
        if resp.status_code in (403, 404):
            return [self._unavailable(
                "employment_hero.roster.terminated_still_active",
        ...
        return [IntegrationFinding(
            check_id="employment_hero.roster.terminated_still_active",
            title="Terminated employees promptly marked inactive",
            description=(
                f"{len(employees)} terminated employee record(s) reviewed; "
                f"{len(still_active)} still show an active status in Employment Hero."
            ),
            remediation=(
                "Deactivate terminated employees in Employment Hero immediately "
                "upon offboarding so downstream access-review evidence stays accurate."
            ),
    ...
    async def _check_manager_assignment_coverage(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        ...
        if resp.status_code in (403, 404):
            return [self._unavailable(
                "employment_hero.roster.manager_assignment_coverage",
        ...
        return [IntegrationFinding(
            check_id="employment_hero.roster.manager_assignment_coverage",
            ...
            remediation=(
                "Assign a manager to every active employee in Employment Hero "
                "so approval and review chains resolve correctly."
            ),
    ...
    async def _check_change_log_availability(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        resp = await self._get(client, "/employees/activity", limit=10, sort="-occurred_at")
        if resp.status_code in (403, 404):
            return [self._unavailable(
                "employment_hero.audit.change_log_availability",
                "Employment record change history is retrievable",
                "audit_logging",
                "Grant the OAuth2 client read access to the activity log.",
            )]
        resp.raise_for_status()
        data = resp.json()
        ...
        return [IntegrationFinding(
            check_id="employment_hero.audit.change_log_availability",
            title="Employment record change history is retrievable",
            description=(
                f"{len(entries)} recent employment change record(s) retrieved from Employment Hero."
                if has_recent else
                "The activity log endpoint returned no recent entries."
            ),
            remediation=(
                "Confirm employment record changes (status, manager, "
                "compensation) are being logged in Employment Hero."
            ),
    ...
    def _unavailable(check_id: str, title: str, category: str, remediation: str) -> IntegrationFinding:
        return IntegrationFinding(
            ...
            description="Sentinel could not read this from Employment Hero with the supplied credentials.",
