...
"""VMware Workspace ONE UEM integration adapter (legacy branding).

Uses httpx against the Workspace ONE UEM REST API -- same API as the Omnissa
adapter but retained under the VMware branding for orgs that have not yet
migrated their catalogue slug.

Auth: API key (aw-tenant-code) plus admin credentials (read-only).

Evidence source: devices, profiles, compliance policies.
...
+-----------------------------------------------------+---------------------------+--------------------------------------------+
| vmware_workspace_one.devices.compliance_status      | endpoint_protection       | SOC2 CC6.8 . ISO27001 A.8.1               |
| vmware_workspace_one.devices.encryption_status      | encryption_at_rest        | SOC2 CC6.1 . ISO27001 A.10.1.1 . PCI 3.4  |
| vmware_workspace_one.profiles.assignment_status     | access_control            | SOC2 CC6.1 . ISO27001 A.12.5.1            |
+-----------------------------------------------------+---------------------------+--------------------------------------------+
"""
...
@dataclass
class VMwareWorkspaceOneCredentials:
    """Matches dashboard/src/integrations/vmware_workspace_one/config.ts credentialFields."""
...
class VMwareWorkspaceOneAdapter:
    """Fetches device posture from VMware Workspace ONE UEM (legacy branding).

    No database access; the worker persists returned findings.
    """

    def __init__(self, credentials: VMwareWorkspaceOneCredentials, client: httpx.AsyncClient | None = None) -> None:
    ...
    async def fetch_all(self) -> list[IntegrationFinding]:
        ...
        for result in results:
            if isinstance(result, BaseException):
                logger.warning("vmware_workspace_one check failed: %s", result)
                continue
    ...
    async def _check_compliance(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        ...
        if resp.status_code == 403:
            return [self._unavailable(
                "vmware_workspace_one.devices.compliance_status",
        ...
        return [IntegrationFinding(
            check_id="vmware_workspace_one.devices.compliance_status",
    ...
    async def _check_encryption(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        ...
        if resp.status_code == 403:
            return [self._unavailable(
                "vmware_workspace_one.devices.encryption_status",
        ...
        return [IntegrationFinding(
            check_id="vmware_workspace_one.devices.encryption_status",
    ...
    async def _check_profile_assignment(self, client: httpx.AsyncClient) -> list[IntegrationFinding]:
        ...
        if resp.status_code in (403, 404):
            return [self._unavailable(
                "vmware_workspace_one.profiles.assignment_status",
        ...
        return [IntegrationFinding(
            check_id="vmware_workspace_one.profiles.assignment_status",
