...
export default function EvidenceSyncEngine() {
  ...
  useEffect(() => {
    fetch('/api/compliance/evidence', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.ok ? r.json() : [])
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground dark:text-foreground">Evidence Sync Engine</h1>
          <p className="text-sm text-muted-foreground mt-1">Evidence collection and auto-sync with freshness indicators</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white dark:bg-card rounded-none border border-border dark:border-border">
          ...
          <p className="text-sm text-muted-foreground mt-2">Create your first evidence sync engine to get started</p>
        </div>
      ) : (
