...
export default function HitQueue() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('hit_queue').select('*').order('created_at', { ascending: false });
    setItems(data || []);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">HIT Queue</h1>
        <Button onClick={fetchItems}>Refresh</Button>
      </div>
      <p className="text-muted-foreground">Human-in-the-loop task queue for review items.</p>
      ...
      <Card>
        <CardHeader><CardTitle>HIT Queue Records</CardTitle></CardHeader>
        <CardContent>
