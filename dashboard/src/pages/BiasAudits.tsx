...
export default function BiasAudits() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('bias_audits').select('*').order('created_at', { ascending: false });
    setItems(data || []);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Bias Audits</h1>
        <Button onClick={fetchItems}>Refresh</Button>
      </div>
      <p className="text-muted-foreground">Automated bias detection and fairness auditing for AI models.</p>
      ...
      <Card>
        <CardHeader><CardTitle>Bias Audits Records</CardTitle></CardHeader>
        <CardContent>
