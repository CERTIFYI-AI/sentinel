...
export default function Guardrails() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('guardrails').select('*').order('created_at', { ascending: false });
    setItems(data || []);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-muted-foreground">Define and enforce AI safety guardrails and constraints.</p>
      ...
      <Card>
        <CardHeader><CardTitle>Guardrails Records</CardTitle></CardHeader>
        <CardContent>
