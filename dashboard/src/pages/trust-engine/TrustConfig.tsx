...
export default function TrustConfig() {
  ...
  useEffect(() => {
    fetch('/api/trust-engine/config', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.ok ? r.json() : [])
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground dark:text-foreground">Trust Configuration</h1>
          <p className="text-sm text-muted-foreground mt-1">Per-model trust configuration: intent boundaries, schemas, fallback chains</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white dark:bg-card rounded-none border border-border dark:border-border">
          ...
          <p className="text-sm text-muted-foreground mt-2">Create your first trust configuration to get started</p>
        </div>
      ) : (
