...
export default function TrustEngineDashboard() {
  ...
  const trustKpiCards: StatCardRowItem[] = [
    ...
    },
    {
      ...
      value: analytics?.total14d != null ? formatNumber(analytics.total14d) : '—',
      icon: <Lightning size={18} weight="fill" style={{ color: 'hsl(var(--s-in-tx))' }} />,
      description: analytics?.total14d != null ? `Traces evaluated in the last 14 days: ${analytics.total14d}` : 'No trace data available',
    },
    {
      label: 'Violations (7d)',
      value: analytics?.blocked7d != null ? String(analytics.blocked7d) : '—',
      icon: <Warning size={18} weight="fill" style={{ color: 'hsl(var(--s-wn-tx))' }} />,
      description: analytics?.blocked7d != null ? `Blocked traces in the last 7 days: ${analytics.blocked7d}` : 'No violation data available',
