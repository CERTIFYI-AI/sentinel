...
export default function ToolCallMonitor() {
  ...
  useEffect(() => {
    fetch('/api/trust-engine/tool-calls', { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.ok ? r.json() : [])
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground dark:text-foreground">Tool Call Monitor</h1>
          <p className="text-sm text-muted-foreground mt-1">Agent tool call monitoring with allow/block status</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white dark:bg-card rounded-none border border-border dark:border-border">
          ...
          <p className="text-sm text-muted-foreground mt-2">Create your first tool call monitor to get started</p>
        </div>
      ) : (
