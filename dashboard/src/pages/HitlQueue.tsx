...
export default function HitlQueue() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('hitl_queue').select('*').order('created_at', { ascending: false });
    setItems(data || []);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-muted-foreground">Human-in-the-loop processing queue management.</p>
      ...
      <Card>
        <CardHeader><CardTitle>HITL Queue Records</CardTitle></CardHeader>
        <CardContent>
