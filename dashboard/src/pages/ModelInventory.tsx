...
export default function ModelInventory() {
  ...
  if (isLoading) return <div className="p-8"><div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-48 rounded-none bg-muted animate-pulse" />)}</div></div>;
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {selected.size >= 2 && <button className="h-9 px-3 rounded-none border border-border text-sm hover:bg-muted">Compare ({selected.size})</button>}
          <button className="h-9 px-3 rounded-none bg-[#1A6B5A] text-foreground text-sm font-medium hover:bg-[#1A6B5A]/90 flex items-center gap-2"><Plus className="w-4 h-4" />Add Model</button>
        </div>
      </div>
        ...
        <div className="flex gap-1">
          {providers.map(p => (
            <button key={p} onClick={() => setProviderFilter(p)}
              className={`px-3 py-1.5 text-xs rounded-none capitalize transition-colors ${providerFilter === p ? "bg-[#1A6B5A] text-foreground" : "text-muted-foreground hover:bg-muted"}`}>{p}</button>
          ))}
        </div>
        <div className="flex items-center gap-2">
          <div className="relative"><Search className="absolute left-2.5 top-2.5 w-4 h-4 text-muted-foreground" />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search models..."
              className="h-9 w-56 rounded-none border border-input bg-background pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
          </div>
          <div className="flex border border-border rounded-none">
            <button onClick={() => setView("grid")} className={`p-2 ${view === "grid" ? "bg-muted" : ""}`}><LayoutGrid className="w-4 h-4" /></button>
            <button onClick={() => setView("table")} className={`p-2 ${view === "table" ? "bg-muted" : ""}`}><Table className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
