...
const priorityColors:Record<string,string> = {critical:"bg-red-500/10 text-red-600",high:"bg-amber-500/10 text-amber-600",medium:"bg-blue-500/10 text-blue-600",low:"bg-slate-200 text-slate-600"};
const statusColors:Record<string,string> = {open:"bg-slate-200 text-slate-600","in-progress":"bg-blue-500/10 text-blue-600",resolved:"bg-emerald-500/10 text-emerald-600",deferred:"bg-amber-500/10 text-amber-600"};
  ...
  return (
    <div className="p-6 bg-slate-50 dark:bg-slate-950 min-h-screen">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Remediation Tracker</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        {[{l:"Total",v:stats.total,c:"text-slate-900"},{l:"Open",v:stats.open,c:"text-amber-600"},{l:"In Progress",v:stats.inProgress,c:"text-blue-600"},{l:"Resolved",v:stats.resolved,c:"text-emerald-600"}].map((s,i)=>(
          <Card key={i}><CardContent className="p-4"><div className="text-xs text-slate-500 mb-1">{s.l}</div><div className={`text-2xl font-bold ${s.c} dark:text-white`}>{s.v}</div></CardContent></Card>
        ))}
      </div>
          ...
          <Card key={item.id} className="hover:ring-1 hover:ring-blue-500/30 transition">
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                ...
                <button className="p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800"><ArrowUpRight size={14} className="text-slate-400"/></button>
              </div>
            </CardContent>
          </Card>
        ))}
