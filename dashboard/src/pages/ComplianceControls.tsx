...
export default function ComplianceControls() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('compliance_controls').select('*').order('created_at', { ascending: false });
    setItems(data || []);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Compliance Controls</h1>
        <Button onClick={fetchItems}>Refresh</Button>
      </div>
      <p className="text-muted-foreground">Manage and track compliance control implementations.</p>
      ...
      <Card>
        <CardHeader><CardTitle>Compliance Controls Records</CardTitle></CardHeader>
        <CardContent>
