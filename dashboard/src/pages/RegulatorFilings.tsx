...
import { useRegulatorFilings } from "@/hooks/queries/useRegulatorFilings";
import { MOCK_REGULATOR_FILINGS } from "@/data/assets";
...
export default function RegulatorFilings() {
  ...
  const data = MOCK_REGULATOR_FILINGS;
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Regulator Filing Workspace</h1>
          <p className="text-sm text-muted-foreground mt-1">Regulatory notification management: NIS2, DORA, GDPR, EU AI Act</p>
        </div>
        <Button onClick={() => setShowCreate(true)}>
          <Plus weight="bold" className="mr-2 h-4 w-4" /> New Filing
        </Button>
      </div>
      ...
      <div className="rounded-xl border bg-card">
        <table className="w-full">
          <thead>
            <tr className="border-b text-left">
              <th className="px-4 py-3 text-xs font-medium text-muted-foreground">Filing Ref</th>
              <th className="px-4 py-3 text-xs font-medium text-muted-foreground">Title</th>
              <th className="px-4 py-3 text-xs font-medium text-muted-foreground">Regulation</th>
              <th className="px-4 py-3 text-xs font-medium text-muted-foreground">Type</th>
              <th className="px-4 py-3 text-xs font-medium text-muted-foreground">Status</th>
              <th className="px-4 py-3 text-xs font-medium text-muted-foreground">Deadline</th>
              <th className="px-4 py-3 text-xs font-medium text-muted-foreground">Regulator</th>
        ...
        <td className="px-4 py-3 text-sm text-muted-foreground">{r.filing_ref}</td>
        <td className="px-4 py-3 text-sm text-muted-foreground">{r.title}</td>
        <td><span className="inline-flex items-center rounded-full px-2 py-1 text-xs font-medium bg-muted">{r.regulation}</span></td>
        ...
        <td className="px-4 py-3 text-sm text-muted-foreground">{r.regulator_name}</td>
