...
import { ErrorState } from '../components/evals/states';
...
export default function ModelRiskCommittee() {
  ...
  const {
    meetings, agenda, votes, members: rosterRows, isLoading, error,
    vote, addMeeting, addAgenda, decide, addMember: addMemberMut, removeMember: removeMemberMut,
  } = useMrc();
  const { models } = useModelOptions();
  const { options: orgUsers } = useUserOptions();

  // Real committee roster (mrc_committee_members) mapped to the view shape the
  // JSX below uses. Quorum is counted from this, not a hardcoded list.
  const members = useMemo(
    () => rosterRows.map(m => ({
      id: m.id, name: m.memberName, role: m.committeeRole,
      department: m.department ?? '—', chair: m.isChair, quorum: m.countsTowardQuorum,
    })),
    [rosterRows],
  );
  ...
  // Committee members — on the real mrc_committee_members table. Writes throw;
  // the toast fires only after the mutation resolves.
  ...
  const addMember = () => {
    const user = orgUsers.find(u => u.id === newMemberId);
    if (!user) { toast.error('Select a user from the organization'); return; }
    if (rosterRows.some(m => m.userId === user.id || m.memberName === user.name)) { toast.error(`${user.name} is already on the committee`); return; }
    addMemberMut.mutate(
      { userId: user.id, memberName: user.name, committeeRole: newMemberRole, department: user.department ?? null, countsTowardQuorum: newMemberQuorum },
      {
        onSuccess: () => {
  ...
  const removeMember = (mid: string, name: string) => {
    removeMemberMut.mutate(mid, {
      onSuccess: () => toast.success(`${name} removed from the committee`),
      onError: (err) => toast.error((err as Error).message),
    });
  };
  const availableUsers = orgUsers.filter(u => !rosterRows.some(m => m.userId === u.id || m.memberName === u.name));
  ...
  return (
    <TooltipProvider>
      <div className="space-y-6">
        ...
        {/* A thrown query renders a real error state, never an empty agenda. */}
                        ...
                        {item.modelId && (
                          <div className="mt-1">
                            {mo ? (
                              <PillLink
                                label={mo.name}
                                onClick={() => navigate(`/models/inventory/${item.modelId}`)}
                              />
                            ) : (
                              // The id does not resolve to a registry model. Never
                              // fall back to the stale denormalised model_name — an
                              // unresolvable id renders "Unavailable".
                              <span className="inline-flex items-center px-2 py-0.5 text-xs font-medium border border-[hsl(var(--border))] bg-sunken text-[hsl(var(--text-4))]">
                                Unavailable
                              </span>
                            )}
                          </div>
          ...
          <div className="space-y-5 py-4">
            <div className="p-4 text-sm bg-raised border border-[hsl(var(--border))] rounded-none">
              ...
              <p className="text-[hsl(var(--text-3))]">
                {voteDialog?.modelId ? <>Model: {modelById.get(voteDialog.modelId)?.name ?? 'Unavailable'} · </> : null}
