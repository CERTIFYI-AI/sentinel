...
export default function HitlReviews() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('hitl_reviews').select('*').order('created_at', { ascending: false });
    setItems(data || []);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-muted-foreground">Human-in-the-loop review management and tracking.</p>
      ...
      <Card>
        <CardHeader><CardTitle>HITL Reviews Records</CardTitle></CardHeader>
        <CardContent>
