    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" onClick={openCreate} style={{borderRadius:0,background:'hsl(var(--brand))',color:'#fff'}}><Plus size={14} className="mr-1" />New Document</Button>
        </div>
      </div>
      {/* KPIs */}
      ...
      <Sheet open={formOpen} onOpenChange={setFormOpen}>
        <SheetContent side="right" className="w-[620px] sm:max-w-[620px] overflow-y-auto p-0" style={{borderRadius:0}}>
              ...
              <FormField label="Linked Frameworks" hint="Link to compliance frameworks for audit traceability.">
                <div className="flex flex-wrap gap-1.5">
                  {FRAMEWORKS_LIST.map(fw => (
                    <button key={fw} onClick={()=>toggleFramework(fw)} className="px-2 py-1 text-[11px] border transition-colors" style={{borderRadius:0, background:(form.frameworks||[]).includes(fw)?'hsl(var(--brand))':'transparent', color:(form.frameworks||[]).includes(fw)?'#fff':'hsl(var(--text-2))', borderColor:(form.frameworks||[]).includes(fw)?'hsl(var(--brand))':'hsl(var(--border))'}}>{fw}</button>
                  ))}
                </div>
              </FormField>
              {/* Category */}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                {[['template','Policy Template'],['upload','Upload File'],['link','External Link']].map(([val,label]) => (
                  <button key={val} onClick={()=>setF('documentSource')(val)} className="flex-1 px-3 py-2.5 text-xs font-medium border transition-colors text-center" style={{borderRadius:0, background:form.documentSource===val?'hsl(var(--brand))':'transparent', color:form.documentSource===val?'#fff':'hsl(var(--text-2))', borderColor:form.documentSource===val?'hsl(var(--brand))':'hsl(var(--border))'}}>{label}</button>
                ))}
              </div>
          ...
          <div className="sticky bottom-0 px-6 py-4 border-t flex justify-end gap-2" style={{background:'hsl(var(--bg-surface))',borderColor:'hsl(var(--border))'}}>
            ...
            <Button size="sm" onClick={handleSave} disabled={saving} style={{borderRadius:0,background:'hsl(var(--brand))',color:'#fff'}}>{saving ? 'Saving...' : editId ? 'Update Policy' : 'Create Policy'}</Button>
          </div>
        </SheetContent>
      </Sheet>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-[400px] p-6" style={{background:'hsl(var(--bg-surface))',border:'1px solid hsl(var(--border))',borderRadius:0}}>
            ...
            <div className="flex justify-end gap-2 mt-4">
              ...
              <Button size="sm" onClick={handleDelete} style={{borderRadius:0,background:'hsl(var(--destructive))',color:'#fff'}}>Delete</Button>
            </div>
          </div>
        </div>
      )}
