...
export default function ComplianceFrameworks() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    const { data } = await supabase.from('compliance_frameworks').select('*').order('created_at', { ascending: false });
    setItems(data || []);
    setLoading(false);
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <p className="text-muted-foreground">Framework management for regulatory compliance.</p>
      ...
      <Card>
        <CardHeader><CardTitle>Compliance Frameworks Records</CardTitle></CardHeader>
        <CardContent>
