...
const statusColor: Record<string,string> = {
  ...
  approved: "bg-[hsl(var(--brand))]/20 text-emerald-300",
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-card border rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-muted-foreground">Avg Risk Score</p><p className="text-3xl font-bold text-[hsl(var(--brand))]">{avgRisk}</p></div>
            <TrendingUp className="h-8 w-8 text-[hsl(var(--brand))] opacity-70" />
          </div>
          ...
          <tbody>
            {filtered.map(item => (
              <tr key={item.id} className="border-b hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className={`text-xs ${item.sla === "Overdue" ? "text-red-400" : item.sla === "Completed" ? "text-[hsl(var(--brand))]" : "text-muted-foreground"}`}>
                    {item.sla}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <div className="w-12 h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${item.riskScore > 85 ? "bg-red-500" : item.riskScore > 70 ? "bg-orange-500" : "bg-[hsl(var(--brand))]"}`}
