import { Gear, Plus, MagnifyingGlass } from '@phosphor-icons/react';
  ...
  return (
    <div className="p-6 space-y-6 min-h-screen">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 rounded-none bg-green-600 hover:bg-green-700 text-foreground text-sm font-medium transition-colors">
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`w-full flex items-center gap-2 px-3 py-2 rounded-none text-sm transition-colors ${
                activeTab === t.id ? "bg-green-600/20 text-green-400 font-medium" : "text-muted-foreground hover:bg-muted"
              }`}>
        ...
        <div className="flex-1 space-y-4">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div><p className="text-sm font-medium">Theme</p><p className="text-xs text-muted-foreground">Switch between light and dark mode</p></div>
                    <button onClick={toggle} className="p-2 rounded-none border border-border hover:bg-muted transition-colors">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-3 gap-4">
                    <div><label className="text-xs text-muted-foreground">Trust Pass Threshold</label>
                      <input value={trustThreshold} onChange={e => setTrustThreshold(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-none border border-border bg-background text-sm" /></div>
                    <div><label className="text-xs text-muted-foreground">Drift Alert Threshold</label>
                      <input value={driftAlert} onChange={e => setDriftAlert(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-none border border-border bg-background text-sm" /></div>
                    <div><label className="text-xs text-muted-foreground">Drift Block Threshold</label>
                      <input value={driftBlock} onChange={e => setDriftBlock(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-none border border-border bg-background text-sm" /></div>
                  </div>
                </CardContent>
              </Card>
            </>
          ...
          {activeTab === "api-keys" && (
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="p-4 rounded-none border border-border">
                  <div className="flex items-center justify-between">
                    <div><p className="text-sm font-medium font-mono">sk-sentinel-****-****-7f3a</p>
                      <p className="text-xs text-muted-foreground">Created 2 days ago &middot; Last used 1 hour ago</p></div>
                ...
                <button className="flex items-center gap-2 px-4 py-2 rounded-none border border-dashed border-border text-sm text-muted-foreground hover:border-green-500 hover:text-green-400 transition-colors">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between py-2">
                  <div><p className="text-sm font-medium">Email Alerts</p><p className="text-xs text-muted-foreground">Receive compliance and HITL alerts via email</p></div>
                ...
                <div className="flex items-center justify-between py-2">
                  <div><p className="text-sm font-medium">Slack Integration</p><p className="text-xs text-muted-foreground">Post alerts to a Slack channel</p></div>
                ...
                <div><label className="text-xs text-muted-foreground">Webhook URL</label>
                  <input value={webhookUrl} onChange={e => setWebhookUrl(e.target.value)} placeholder="https://hooks.slack.com/services/..." className="w-full mt-1 px-3 py-2 rounded-none border border-border bg-background text-sm" />
                </div>
              </CardContent>
            </Card>
          )}
          ...
          {activeTab === "data" && (
            <Card>
              ...
              <CardContent className="space-y-4">
                <div><label className="text-xs text-muted-foreground">Evidence Retention (days)</label>
                  <input value={retentionDays} onChange={e => setRetentionDays(e.target.value)} className="w-full mt-1 px-3 py-2 rounded-none border border-border bg-background text-sm" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 rounded-none border border-border">
                    <p className="text-xs text-muted-foreground">Evidence Records</p>
                    <p className="text-2xl font-bold text-foreground">14,820</p>
                  </div>
                  <div className="p-4 rounded-none border border-border">
                    <p className="text-xs text-muted-foreground">Storage Used</p>
                    <p className="text-2xl font-bold text-foreground">2.3 GB</p>
                  </div>
                </div>
                <button className="flex items-center gap-2 px-4 py-2 rounded-none border border-red-500/30 text-red-400 text-sm hover:bg-red-500/10 transition-colors">
