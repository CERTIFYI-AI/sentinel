...
export default function EvalRunWizard() {
  ...
  return (<div className="p-6 max-w-3xl mx-auto"><h1 className="text-2xl font-bold mb-1">Eval Run Wizard</h1><p className="text-sm text-muted-foreground mb-6">Configure and launch evaluation runs</p><div className="flex gap-2 mb-6">{steps.map((s,i)=>(<div key={s} className={`flex-1 h-2 rounded ${i<=idx?"bg-[hsl(var(--primary))]":"bg-[hsl(var(--muted))]"}`}/>))}</div>
