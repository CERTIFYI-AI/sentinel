      ...
      {sections.map(s => (
        <NavLink key={s.to} to={`/settings/${s.to}`} end
          className={({ isActive }) => `flex items-center gap-2 px-3 py-2 text-sm rounded-md transition-colors ${
            isActive ? "bg-[hsl(var(--brand-subtle))] text-[hsl(var(--brand-foreground))] border-l-2 border-primary" : "text-gray-500 hover:text-foreground hover:bg-gray-100"
          }`}>
          <s.icon className="w-4 h-4" />{s.label}
        </NavLink>
      ))}
      <div className="pt-2 border-t border-border mt-2">
        <div className="flex items-center gap-2 px-3 py-2 text-sm text-gray-500/50 cursor-not-allowed">
