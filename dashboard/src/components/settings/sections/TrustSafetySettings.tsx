...
export function TrustSafetySettings() {
  ...
  return (
    <div className="space-y-8 max-w-lg">
      ...
      <div className="flex gap-2">{presets.map(p => <button key={p.label} onClick={() => { setThreshold(p.v); setDirty(true); }} className={`text-xs px-3 py-1 rounded border ${threshold === p.v ? "border-primary bg-[#1A6B5A]/10 text-[#1A6B5A]" : "border-border hover:bg-gray-100"}`}>{p.label}</button>)}</div>
      ...
      <div><label className="text-sm font-medium">Semantic drift alert (&sigma;)</label><p className="text-xs text-gray-500">Alert when response embedding drifts this far from baseline.</p>
      ...
      <div><label className="text-sm font-medium">HITL canned response</label>
        ...
        <p className="text-xs text-gray-500 text-right">{hitlResp.length} / 500</p>
      </div>
      ...
      <button onClick={save} disabled={!dirty || update.isPending} className="h-10 px-6 rounded-md bg-[#1A6B5A] text-white text-sm font-medium hover:bg-[#1A6B5A]/90 disabled:opacity-50 flex items-center gap-2">
