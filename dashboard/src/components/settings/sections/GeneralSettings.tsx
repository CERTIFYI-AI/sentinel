...
export function GeneralSettings() {
  ...
  if (isLoading) return <div className="space-y-4">{[1,2,3].map(i => <div key={i} className="h-16 bg-muted animate-pulse rounded-none" />)}</div>;
  ...
  return (
    <div className="space-y-6 max-w-lg">
      ...
      <div><label className="text-sm font-medium">Organization name</label>
        <input value={form.org_name} onChange={e => change("org_name", e.target.value)} className="mt-1 w-full h-10 rounded-none border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
      </div>
      <div><label className="text-sm font-medium">Display name</label>
        <input value={form.display_name} onChange={e => change("display_name", e.target.value)} className="mt-1 w-full h-10 rounded-none border border-input bg-background px-3 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
      </div>
      <div><label className="text-sm font-medium">Tenant ID</label>
        <div className="flex gap-2 mt-1"><input readOnly value={cfg?.id ?? ""} className="flex-1 h-10 rounded-none border border-input bg-muted px-3 font-mono text-sm" />
