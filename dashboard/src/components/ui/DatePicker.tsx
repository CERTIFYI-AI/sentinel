...
export function DatePicker({ value, onChange, placeholder = "Select date", disablePast = false }: DatePickerProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" onClick={() => setOpen(!open)} className="flex items-center gap-2 px-3 py-2 text-sm border border-[hsl(var(--border))] rounded-md hover:bg-surface-2 w-full text-left">
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1 bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-lg shadow-lg p-3 w-[280px]">
          <div className="flex items-center justify-between mb-3">
            <button type="button" onClick={() => setViewDate(new Date(year, month - 1))} className="p-1 hover:bg-surface-2 rounded"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg></button>
            <span className="text-sm font-medium">{monthNames[month]} {year}</span>
            <button type="button" onClick={() => setViewDate(new Date(year, month + 1))} className="p-1 hover:bg-surface-2 rounded"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></button>
          </div>
            ...
            {days.map((d, i) => {
              ...
              return (
                <button key={i} type="button" disabled={isPast} onClick={() => { onChange(dateStr); setOpen(false); }}
                  className={`text-xs p-1.5 rounded text-center transition-colors ${isSelected ? "bg-[hsl(var(--primary))] text-foreground" : isToday ? "text-[hsl(var(--primary))] font-bold" : "hover:bg-surface-2"} ${isPast ? "opacity-30 cursor-not-allowed" : "cursor-pointer"}`}>
                  {d}
                </button>
              );
            })}
