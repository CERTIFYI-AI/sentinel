...
export function AvatarPicker({ value, onChange, label }: AvatarPickerProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      <button type="button" onClick={() => setOpen(!open)} className="flex items-center gap-2 px-2 py-1 rounded-md border border-[hsl(var(--border))] hover:bg-surface-2 text-sm">
      ...
      {open && (
        <div className="absolute z-50 top-full mt-1 w-[200px] bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-lg shadow-lg p-1">
          ...
          <div className="max-h-[180px] overflow-y-auto">
            {filtered.map(m => (
              <button key={m.id} type="button" onClick={() => { onChange(m); setOpen(false); setSearch(""); }} className="w-full flex items-center gap-2 px-2 py-1.5 rounded hover:bg-surface-2">
          ...
          {value && <button type="button" onClick={() => { onChange(null); setOpen(false); }} className="w-full text-left px-2 py-1.5 text-[11px] text-[hsl(var(--destructive))] hover:bg-surface-2 rounded mt-1 border-t border-[hsl(var(--border))] pt-2">Unassign</button>}
        </div>
      )}
    </div>
  );
