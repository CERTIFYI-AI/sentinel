  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: "hsl(var(--card))", borderRadius: "0.75rem", padding: "1.5rem", width: "400px", fontFamily: "Outfit, sans-serif" }}>
        ...
        <input placeholder="Full Name" value={name} onChange={e => setName(e.target.value)} style={{ width: "100%", padding: "0.5rem", marginBottom: "0.75rem", border: "1px solid hsl(var(--border))", borderRadius: "0.375rem", fontFamily: "Outfit, sans-serif" }} />
        ...
        <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
          ...
          <button onClick={() => agreed && name && onSign(name)} disabled={!agreed || !name} style={{ padding: "0.5rem 1rem", background: agreed && name ? "hsl(var(--primary))" : "hsl(var(--muted))", color: "hsl(var(--primary-foreground))", border: "none", borderRadius: "0.375rem", cursor: agreed && name ? "pointer" : "not-allowed", fontFamily: "Outfit, sans-serif" }}>Sign</button>
        </div>
      </div>
    </div>
  );
