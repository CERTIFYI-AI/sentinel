  ...
  return (
    <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "480px", background: "hsl(var(--card))", borderLeft: "1px solid hsl(var(--border))", zIndex: 50, padding: "1.5rem", overflowY: "auto", fontFamily: "Outfit, sans-serif" }}>
      ...
      <div style={{ display: "flex", gap: "0.5rem" }}>
        <button onClick={onApprove} style={{ flex: 1, padding: "0.5rem", background: "hsl(var(--success, 142 76% 36%))", color: "white", border: "none", borderRadius: "0.375rem", cursor: "pointer", fontFamily: "Outfit, sans-serif" }}>Approve</button>
        <button onClick={() => onReject("Needs revision")} style={{ flex: 1, padding: "0.5rem", background: "hsl(var(--destructive, 0 84% 60%))", color: "white", border: "none", borderRadius: "0.375rem", cursor: "pointer", fontFamily: "Outfit, sans-serif" }}>Reject</button>
      </div>
    </div>
  );
}
