  ...
  it('ships a form for each provider the server accepts', () => {
    ...
    expect(INTEGRATIONS.map(i => i.id).sort()).toEqual(
      ['1password', '1password_device_trust_kolide',
       'addigy', 'anthropic_claude_api', 'anthropic_claude_console',
       ...
       'kandji_iru', 'keeper', 'lakera_protect_ai', 'langsmith_langfuse',
       ...
       'ninjaone', 'okta', 'omnissa_workspace_one',
       'one_identity', 'onelogin',
       'openai', 'openai_azure_openai',
       'pinecone', 'ping_identity', 'pingone',
       'sailpoint', 'vmware_workspace_one',
       'weaviate', 'weights_biases_w_b'],
    )
  })
