...
import { SEED_TRAINING_COURSES } from '../data/seedData'

export async function fetchAllTraining(filters: Record<string,any> = {}) {
  if (!isSupabaseConfigured()) return SEED_TRAINING_COURSES
  try {
    let q = supabase.from('training_courses').select('*').order('created_at', { ascending: false })
    if (filters.status) q = q.eq('status', filters.status)
    if (filters.type) q = q.eq('type', filters.type)
    const { data, error } = await q
    if (error) { console.warn('[trainingService] fetch:', error.message); return SEED_TRAINING_COURSES }
    return data && data.length > 0 ? data : SEED_TRAINING_COURSES
  } catch { return SEED_TRAINING_COURSES }
}
