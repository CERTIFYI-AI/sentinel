    ...
    "entries": [
      {
        ...
        "compliance": [
          ...
          "EU AI Act — Art. 12 (record-keeping): the registry is the anchor for traceability. Closed (was TD-018): although modelService.ts still makes no logAction call, every ai_models insert/update/delete is now captured DB-side by trigger trg_audit_ai_models → fn_audit_governed() (migration 20260902000001), which writes an append-only audit_log row with the actor (from the caller's JWT), the org, before/after values, and the action. Because it fires in the database, it also captures direct-SQL and service-role writes the app layer would miss. (Correction to the original TD-018 note: no shared fn_audit_trigger existed on the live project to bolt on, and model_inventory's trigger only ever lived in an unshipped migration — so the fix was to author the trigger, not attach an existing one.) In addition, some detail-page actions (linking a document, saving an alert config, initiating a review, exporting a package) also log to model_activity with the real actor.",
        ...
        "operations": [
          ...
          "TD-018 — ai_models writes are now audit-logged DB-side (migration 20260902000001); the sibling registries use_cases / datasets / risks remain un-audited (blocked on a tenant_id→org_id schema migration). See docs/reference/technical-debt.md.",
          "Unmeasured metrics render —, never a fabricated 0 (FIXED) — recordToModel preserves a null fairness_score as null, and accuracy/latencyMs are now null too (the ai_models registry has no such columns; they are only known once useModelAnalytics telemetry lands). Both detail KPI rows and the fairness list badge/CSV render — in a neutral colour rather than a red 0% BELOW THRESHOLD / 0ms. The create dialog likewise starts a new model with null metrics and monthlyInferences: '—'.",
  ...
  },
  {
    ...
    "entries": [
      ...
      },
      {
        ...
        "parentLabel": null,
        "hasDoc": true,
        "docPath": "docs/modules/genai-risk-profiles.md",
        "title": "GenAI Risk Profiles",
        "purpose": "Model-level generative-AI risk catalogue aligned to the NIST AI 600-1 taxonomy, with severity assessment, guardrail coverage tracking, and mitigation workflow.",
        "why": "NIST AI 600-1 defines twelve risk categories specific to generative AI (confabulation, data privacy, bias/discrimination, harmful content, IP, cybersecurity, data poisoning, interpretability, human-AI confusion, dual use, environmental impact, misuse facilitation). EU AI Act Art. 9 requires a risk management system proportionate to the AI system's risk level. Without a structured catalogue mapping each model to its applicable GenAI risks, organisations cannot demonstrate they have identified, assessed, and mitigated the risks unique to generative AI.",
        "how": [
        ...
        "fields": [
          ...
          [
            ...
            "text",
            "auto",
            "DB default current_user_org_id()::text"
          ],
          [
            "doc",
            "jsonb",
            "yes",
            "Full GenAIRiskProfile entity"
          ],
          [
            "state",
            "text",
            "no",
            "First-class column for indexing"
          ],
          [
            "model_id",
            "text",
            "no",
            "First-class column — ai_models.id (uuid)"
          ],
          [
            "deleted_at",
            "timestamptz",
          ...
          [
            ...
            "integer",
            "yes",
            "Default 1; optimistic concurrency"
          ],
          [
            "created_at",
            "timestamptz",
          ...
          [
            "riskCategory",
            "string",
            "One of 12 NIST AI 600-1 risk names"
          ],
          [
            "riskNumber",
            "number",
          ...
          [
            "guardrails",
            "string",
            "Comma-separated guardrail names; \"None\" if empty"
          ],
          [
            "guardrailCoverage",
            "enum",
            "None / Partial / Implemented"
          ],
          [
            "mitigationStatus",
            "enum",
            "Implemented / Partial / Under Review / Not Addressed"
          ],
          [
            "owner",
            "string",
            "From the central Users directory"
          ],
          [
            "mitigationEvents",
            "array?",
            "{ date, action, user } timeline entries"
          ]
        ],
        "noDocReason": null
      },
  ...
  },
  {
    ...
    "entries": [
      ...
      },
      {
        ...
        "why": "The catalogue held 219 products and nothing in the product read it. The evidence tables behind it — integration_findings, control_finding_evidence, background_jobs — had zero readers too. So the platform had a real collection pipeline, a real control-mapping engine, and no way for a user to reach any of it. This module closes that gap. It also carries an honesty obligation. Of the 217 catalogued products, six ship an adapter today — github, aws, microsoft_azure, okta, microsoft_entra_id and microsoft_entra_id_gcc_high (the latter two share one adapter across sovereign clouds). Rendering a Conn",
        "how": [
        ...
        "fields": [
          ...
          [
            ...
            "the product's own identifiers (every catalogued product) plus owner, cadence, evidence location"
          ],
          [
            "Secrets",
