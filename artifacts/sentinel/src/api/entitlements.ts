...
export const entitlementsApi = {
  ...
  create: async (data: any) => {
    ...
    await logAction({ module: MODULE, entityType: 'entitlement', entityId: result.id, action: 'create', newValues: result });
    return result;
  },
  update: async (id: string, data: any) => {
    ...
    await logAction({ module: MODULE, entityType: 'entitlement', entityId: id, action: 'update', newValues: data });
    return result;
  },
  delete: async (id: string, name?: string) => {
    await mutateDB(() => supabase.from(TABLE).delete().eq('id', id), null);
    await logAction({ module: MODULE, entityType: 'entitlement', entityId: id, entityName: name, action: 'delete' });
  },
};
