...
export const igaApi = {
  ...
  create: async (data: any) => {
    ...
    await logAction({ module: MODULE, entityType: 'access-review', entityId: result.id, action: 'create', newValues: result });
    return result;
  },
  update: async (id: string, data: any) => {
    ...
    await logAction({ module: MODULE, entityType: 'access-review', entityId: id, action: 'update', newValues: data });
    return result;
  },
  delete: async (id: string, name?: string) => {
    await mutateDB(() => supabase.from(TABLE).delete().eq('id', id), null);
    await logAction({ module: MODULE, entityType: 'access-review', entityId: id, entityName: name, action: 'delete' });
  },
};
