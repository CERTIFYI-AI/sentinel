...
export const biaApi = {
  ...
  create: async (data: any) => {
    ...
    await logAction({ module: MODULE, entityType: 'bia-record', entityId: result.id, action: 'create', newValues: result });
    return result;
  },
  update: async (id: string, data: any) => {
    ...
    await logAction({ module: MODULE, entityType: 'bia-record', entityId: id, action: 'update', newValues: data });
    return result;
  },
  delete: async (id: string, name?: string) => {
    await mutateDB(() => supabase.from(TABLE).delete().eq('id', id), null);
    await logAction({ module: MODULE, entityType: 'bia-record', entityId: id, entityName: name, action: 'delete' });
  },
};
