...
const TABLE = 'sod_violations';
const MODULE = 'iga';

export const sodViolationsApi = {
  ...
  create: async (data: any) => {
    ...
    await logAction({ module: MODULE, entityType: 'sod-violation', entityId: result.id, action: 'create', newValues: result });
    return result;
  },
  update: async (id: string, data: any) => {
    ...
    await logAction({ module: MODULE, entityType: 'sod-violation', entityId: id, action: 'update', newValues: data });
    return result;
  },
  delete: async (id: string, name?: string) => {
    await mutateDB(() => supabase.from(TABLE).delete().eq('id', id), null);
    await logAction({ module: MODULE, entityType: 'sod-violation', entityId: id, entityName: name, action: 'delete' });
  },
};
