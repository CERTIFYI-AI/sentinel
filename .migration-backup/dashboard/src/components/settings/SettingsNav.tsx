    ...
    <nav className="w-[200px] shrink-0 space-y-1">
      {sections.map(s => (
        <NavLink key={s.to} to={`/settings/${s.to}`} end
          className={({ isActive }) => `flex items-center gap-2 px-3 py-2 text-sm rounded-none transition-colors ${
            isActive ? "bg-[hsl(var(--brand-subtle))] text-[hsl(var(--brand-foreground))] border-l-2 border-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted"
          }`}>
