...
- **EU AI Act Art. 12** — record-keeping. The registry is the anchor an auditor walks back from. Every `ai_models` insert/update/delete is now written to `audit_log` by the DB trigger `trg_audit_ai_models` (migration `20260902000001`), so the record survives even a direct or service-role write.
